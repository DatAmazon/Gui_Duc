...
namespace giadinhthoxinh.Controllers
{
    public class UserController : Controller
    {
        ...
        public ActionResult Login(string sEmail, string sPass)
        {
            ...
            if (ktraadmin.Count>0 && ktraadmin[0].FK_iPermissionID>2)
            {
                Session["Admin"] = ktraadmin[0];
                var tmp = (tblUser)Session["Admin"];
                if (tmp.FK_iPermissionID == 3)// id quyen bang 2 thi chi co quyen nhan vien
                {
                    Session["Nhanvien"] = Session["Admin"];
                }
                if (tmp.FK_iPermissionID == 8)// id quyen bang 2 thi co quyen admin
                {
                    Session["Nhanvien"] = Session["Admin"];
